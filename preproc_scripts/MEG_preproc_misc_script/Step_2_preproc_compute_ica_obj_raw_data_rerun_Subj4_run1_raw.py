...
if True:
    ...
    if True:
        ...
        raw.save(filter_dir + "%s/%s_%s_%s_old.fif" %( subj, subj, run, fname_suffix))
        raw1 = raw.crop(tmin = 0.0, tmax = 400.0)
        raw1.save(fif_name, overwrite = True)
    
        # ============= ICA ================================================
        ica = ICA(n_components=0.999, max_pca_components=None)
        # which sensors to use
        picks = mne.pick_types(raw1.info, meg=True, eeg=False, eog=False,ecg = False,
        ...
        ica.fit(raw1, picks=picks, decim=10) 
        print time.time()-tmp_time
    	
        #===== EOG ======
        # create EOG epochs to improve detection by correlation
        picks = mne.pick_types(raw1.info, meg=True, eog=True)
        eog_epochs = create_eog_epochs(raw1, picks=picks)
        ...
        #====== ECG ======
        ecg_epochs = create_ecg_epochs(raw1, picks=picks)
